...
import service.movies.core.GenreRecords;
...
public class GenrePage {
    ...
    public Response getGenres(@Context HttpHeaders headers) {
        GetGenresResponseModel responseModel;

        String email = headers.getHeaderString("email");
        String sessionID = headers.getHeaderString("sessionID");
        String transactionID = headers.getHeaderString("transactionID");

        if(!MovieRecords.isUserAllowedToMakeRequest(email, 3))
        {
            VerifyPrivilegeResponseModel verifyModel = new VerifyPrivilegeResponseModel(141, validate.caseMessage(141));
            return Response.status(Response.Status.OK).header("email", email).
                    header("sessionID", sessionID).
                    header("transactionID", transactionID).
                    entity(verifyModel).build();
        }

        ServiceLogger.LOGGER.info("Received request to get genres.");

        responseModel = GenreRecords.getGenresFromDb(email);
    ...
    public Response addGenre(@Context HttpHeaders headers, String jsonText) {
        ...
        try {
            ...
            responseModel = GenreRecords.addGenreToDb(requestModel);
            if (responseModel.getResultCode() == -1) {
                return Response.status(Response.Status.INTERNAL_SERVER_ERROR).
                        header("email", email).
                        header("sessionID", sessionID).
    ...
    @GET
    @Produces(MediaType.APPLICATION_JSON)
    public Response getGenresFromMovie(@Context HttpHeaders headers,
                                       @PathParam("movieid") String movieid) {
        GetGenresByMovieResponseModel responseModel;
        ...
        ServiceLogger.LOGGER.info("Received request to get genres from movie with id: " + movieid);

        responseModel = GenreRecords.getGenresByMovieFromDb(movieid, email);
        if (responseModel.getResultCode() == -1) {
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).
                    header("email", email).
                    header("sessionID", sessionID).
